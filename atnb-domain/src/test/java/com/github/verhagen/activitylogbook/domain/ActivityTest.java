...
public class ActivityTest {
	...
	void activityWithStartNoStopNoDate() {
		try {
			...
			assertEquals("2020.12.01 14:00", activity.getStartActivity().format(formatterDateTime));
	...
	void activityNoStartWithStopNoDate() {
		try {
			...
			assertEquals("2020.12.01", activity.getDate().format(formatter));
			assertEquals("backing", activity.getIdentifier());
			assertEquals("2020.12.01 16:30", activity.getStopActivity().format(formatterDateTime));
			assertNull(activity.getStartActivity());
		}
		catch (RuntimeException re) {
			fail(re.getMessage());
		}
