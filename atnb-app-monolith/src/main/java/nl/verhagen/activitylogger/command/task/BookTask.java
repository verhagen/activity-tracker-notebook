...
//		}
	...
	public void execute(String[] identifierPath, String command, Map<String, Object> fields) {
		if (getTaskIdentiefersPure().isUniqueIdentifierRequired()) {
			int indexOfTaskIdentifier = -1;
			for (int index = 0; index < identifierPath.length; index++) {
				if (getTaskIdentiefersPure().getIdentifier().equals(identifierPath[index])) {
			...
			if (indexOfTaskIdentifier > -1) {
				if (isLastElementOfArray(identifierPath, indexOfTaskIdentifier)) {
					throw new TaskHandlerException("Argument 'identifierPath' with value '" + Arrays.asList(identifierPath)
							+ "' does contain the task identifier '" + getTaskIdentiefersPure().getIdentifier() + "', but requires an unique identifier.");
				}
			}
		}
		if (! getTaskIdentiefersPure().isKnownTaskCommand(command)) {
			throw new TaskHandlerException("Argument 'command' with value '" + command + "' is not a known command. Known commands are: " + getTaskIdentiefersPure().getTaskCommands());
		}
